...
reactNativeWindowsVersion=0.57.0-rc.5
...
nix-shell -p nodejs-10_x yarn --run "cd rnproject && yarn add react-native-cli && yarn add react-native-macos-cli"

# init ios/android/macos project
nix-shell -p nodejs-10_x yarn python2 --run "\
...
nix-shell -p jre8 androidsdk_9_0 which --run "cd rnproject/android;\
...
# android/macos packaging apparently goes through all deps in node_modules, and fails on these synlinks
...
nix-shell -p nodejs-10_x pash --run "cd rnproject && ./node_modules/rnpm/bin/cli windows --windowsVersion $reactNativeWindowsVersion"
...
#sed -i "s/this.flags = flags;/flags = flags || ''; this.flags = flags;/" rnproject/node_modules/react-native/node_modules/@react-native-community/cli/node_modules/commander/index.js
